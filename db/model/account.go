...
var AccountWhere = struct {
	...
	Code:          whereHelpernull_String{field: "\"account\".\"code\""},
...
var (
	accountAllColumns            = []string{"id", "name", "description", "code", "account_type_id"}
	accountColumnsWithoutDefault = []string{"name", "account_type_id"}
	accountColumnsWithDefault    = []string{"id", "description", "code"}
...
func (o *Account) Splits(mods ...qm.QueryMod) splitQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"split\".\"account_id\"=?", o.ID),
	)
...
func (accountL) LoadSplits(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAccount interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`split`),
		qm.WhereIn(`split.account_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
